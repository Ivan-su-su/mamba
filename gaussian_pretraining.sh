...
# if [ "$train" = "train" ]; then
#     python opencood/tools/train_backbone3d_semantic_pretraining.py \
#         -y "${MODEL_DIR}/config_backbone3d_pretraining.yaml" \
#         --tag semantic_pretrain \
#         --gpu_id ${gpu_id} #\
#         # --amp  # 可选，启用混合精度训练
# fi

if [ "$train" = "train" ]; then
    python opencood/tools/train_backbone2d_semantic_pretraining.py \
        -y "${MODEL_DIR}/config_backbone2d_pretraining.yaml" \
        --tag semantic_pretrain \
        --gpu_id ${gpu_id} #\
        # --amp  # 可选，启用混合精度训练
fi
