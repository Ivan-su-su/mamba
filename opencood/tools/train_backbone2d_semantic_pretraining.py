...
# 
# 预训练脚本：用于预训练 Backbone2D 的语义分类部分
# 训练整个流程：Image Backbone → Detection Head → Semantic Classification
...
def train_parser():
    ...
    """
    parser = argparse.ArgumentParser(description="Backbone2D Semantic Pretraining")
    ...
    parser.add_argument("--tag", default="semantic_pretrain_2d",
        ...
        for _, batch_data in tqdm(enumerate(val_loader),
            ...
            with amp.autocast(enabled=scaler is not None):
                ...
                for agent in available_agent:
                    if agent not in batch_dict:
                        continue
                    if 'semantic_logits' not in batch_dict[agent] or 'semantic_targets' not in batch_dict[agent]:
                        continue
                    agent_logits = batch_dict[agent]['semantic_logits']
                    agent_targets = batch_dict[agent]['semantic_targets']
                    B_agent, N_agent, H_agent, W_agent = agent_targets.shape
                    B_N_agent = B_agent * N_agent
                    if agent_logits.shape[0] != B_N_agent:
                        continue
                    output_dict_agent = {'semantic_logits': agent_logits}
                    target_dict_agent = {'semantic_targets': agent_targets.view(1, B_N_agent, H_agent, W_agent)}
                    val_agent_loss = criterion(output_dict_agent, target_dict_agent)
                    val_loss_list.append(val_agent_loss)
...
def main():
    ...
    # 修改模型创建逻辑，使用预训练版本
    backbone_config = hypes["model"]["args"]["BACKBONE_2D"]
    
    # 导入预训练模型
    from opencood.models.gaussian_modules.backbone2d_semantic_pretraining import GaussianImageBackbone
    
    model = GaussianImageBackbone(model_cfg=backbone_config)
    ...
    print("    1. Image Backbone: 图像特征提取 (GaussianImageFeatureExtractor)")
    print("    2. Detection Head: 2D检测头 (GaussianDetectionHead)")
    print("    3. Semantic Classification: 语义分类头 (输出 num_classes 维 logits)")
    print("  ")
    print("  损失计算：使用 semantic_logits (logits)，不是 class_probs (probs)")
    ...
    from opencood.models.gaussian_modules.backbone2d_semantic_loss import Backbone2DSemanticLoss
    criterion = Backbone2DSemanticLoss(loss_config)
    ...
    for epoch in range(init_epoch, max(epochs, init_epoch)):
        ...
        for i, batch_data in pbar:
            ...
            with amp.autocast(enabled=scaler is not None):
                # 前向传播：Image Backbone → Detection Head → Semantic Classification
                batch_dict = model(batch_data["ego"], available_agent=available_agents)
                
                # 按 agent 分别计算损失（FPN 多尺度下不同 agent 可能分辨率不同，无法 concat）
                loss_list = []
                for agent in available_agents:
                    ...
                    output_dict_agent = {'semantic_logits': agent_logits}
                    target_dict_agent = {'semantic_targets': agent_targets.view(1, B_N_agent, H_agent, W_agent)}
                    agent_loss = criterion(output_dict_agent, target_dict_agent)
                    loss_list.append(agent_loss)
                
                if len(loss_list) == 0:
                    print("[Warning] No valid agent data for loss, skipping batch")
                    continue
