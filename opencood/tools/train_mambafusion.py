...
# 添加 MambaFusion 相关的导入
try:
    from opencood.models.airv2x_mambafusion import Airv2xMambafusion
    MAMBAFUSION_AVAILABLE = True
except ImportError:
    MAMBAFUSION_AVAILABLE = False
    print("Warning: MambaFusion not available")

def is_mambafusion_model(hypes):
        ...
        for _, batch_data in tqdm(enumerate(val_loader),
            ...
            if "scope" in hypes["name"] or "how2comm" in hypes["name"]:
                ...
                with amp.autocast(enabled=scaler is not None):
                    ...
                    if hasattr(model, 'get_training_loss'):
                        loss, tb_dict, disp_dict = model.get_training_loss(out)
                    else:
...
def main():
    ...
    if is_mambafusion_model(hypes) and MAMBAFUSION_AVAILABLE:
        # MambaFusion 需要特殊的模型创建方式
        print("Creating MambaFusion model...")
        model = Airv2xMambafusion(hypes["model"]["args"], dataset=None, num_class=hypes["model"]["args"].get("num_class", 7))
    else:
        ...
        for i, batch_data in pbar:
            ...
            if "scope" in hypes["name"] : #or "how2comm" in hypes["name"]
                ...
                with amp.autocast(enabled=scaler is not None):
                    ...
                    # MambaFusion 的损失计算方式
                    if hasattr(model, 'get_training_loss'):
                        loss, tb_dict, disp_dict = model.get_training_loss(output_dict)
                    else:
