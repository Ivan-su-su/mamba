...
from ..model_utils.unitr_utils_all import MapImage2Lidar, MapLidar2Image
...
class MambaFusion_all(nn.Module):
    ...
    def _merge_lidar2image_multi_view_output(self, expanded_output, base_output, voxel_hit_indices, voxel_num):
        """Reduce multi-view expanded lidar features back to the original voxel layout."""
        lidar_hit_num = int(voxel_hit_indices.shape[0])
        lidar_base = base_output[:voxel_num]
        image_output = expanded_output[lidar_hit_num:]
        lidar_merged = lidar_base.new_zeros((voxel_num, lidar_base.shape[1]))
    ...
    def forward(self, batch_dict, agent=None):
        ...
        # TODO: 这里可以优化，因为batch_dict没有batch_size
        ...
        if self.lidar2image_on: # 将lidar的feature映射到image的feature 将激光雷达数据预处理为图像特征，以增强图像的特征表示
            if self.use_mamba_inter2: #True
                ...
                if self.use_multi_scale: #False
                    lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, lidar2image_coor, lidar2image_coords_bzyx_list, voxel_hit_indices = self._lidar2image_preprocess(
                        batch_dict, multi_feat, multi_pos_embed_list)
                    ...
                    lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, lidar2image_coor, voxel_hit_indices = self._lidar2image_preprocess(
        ...
        for stage_id in range(self.stage_num): # 1
            ...
            for i in range(len(block_layers)): # 4 (1 2 1) use hilbert curve   #一模一样的两层
    ...
    def _extract_camera_imgs(self, batch_dict):
        ...
        """
        if 'batch_merged_cam_inputs' in batch_dict:
            ...
            raise KeyError("No camera images found in batch_dict: expected 'batch_merged_cam_input")
    ...
    def _input_preprocess2(self, batch_dict):
        ...
        if self.patch_coords is not None and ((self.patch_coords[:, 0].max().int().item() + 1) == B*N):
            ...
            batch_dict['patch_coords'] = torch.cat([batch_idx, self.patch_zyx.clone().to(imgs.device)[
                                                   None, ::].repeat(B*N, 1, 1).view(-1, 3)], dim=-1).long()
            self.patch_coords = batch_dict['patch_coords'].clone()   #[num_of_patch, 4]
            # TODO: 以下两个判断都为False，还未查看
            if self.use_multi_scalev:
                new_hw_shape = [size * 2 for size in hw_shape]
                new_batch_idx = torch.arange(
                    B*N, device=imgs.device).unsqueeze(1).repeat(1, new_hw_shape[0] * new_hw_shape[1]).view(-1, 1)
    ...
    def _lidar2image_preprocess(self, batch_dict, multi_feat, multi_pos_embed_list, outer_dict = None):
        ...
        lidar2image_coords_zyx, lidar2image_coords_bzyx_list, voxel_hit_indices = self.map_lidar2image_layer(batch_dict, self.use_multi_scale) # [num_hits, 3] view_idx, x, y 点云投影到图像上的坐标 TODO
