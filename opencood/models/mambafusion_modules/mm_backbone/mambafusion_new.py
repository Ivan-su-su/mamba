...
from ..model_utils.unitr_utils_new import MapImage2Lidar, MapLidar2Image
...
class MambaFusion_new(nn.Module):
    def __init__(self, model_cfg, use_map=False, **kwargs):
        ...
        self.use_lidar2image_relation = model_cfg.get('USE_LIDAR2IMAGE_RELATION', False)
        ...
        if self.fuse_on:
            ...
            if self.lidar2image_on:
                ...
                if self.use_lidar2image_relation:
                    self.lidar2image_relation_mlp = nn.Sequential(
                        nn.Linear(4, 128),
                        nn.LayerNorm(128),
                        nn.ReLU(inplace=True),
                        nn.Linear(128, 128),
                    )
                    self.lidar2image_relation_scale = nn.Parameter(torch.ones(1) * 0.1)
        # new
        block_id = 0
    ...
    def _merge_lidar2image_multi_view_output(self, expanded_output, base_output, voxel_hit_indices, voxel_num):
        """Reduce expanded lidar hits back to the original voxel layout."""
        lidar_hit_num = int(voxel_hit_indices.shape[0])
        lidar_base = base_output[:voxel_num]
        image_output = expanded_output[lidar_hit_num:]
        lidar_merged = lidar_base.clone()

        if lidar_hit_num == 0:
            return torch.cat([lidar_merged, image_output], dim=0)

        lidar_expanded = expanded_output[:lidar_hit_num]
        lidar_sum = lidar_base.new_zeros((voxel_num, lidar_base.shape[1]))
        lidar_count = lidar_base.new_zeros((voxel_num, 1))
        lidar_sum.index_add_(0, voxel_hit_indices, lidar_expanded)
        lidar_count.index_add_(
            0,
            voxel_hit_indices,
            lidar_base.new_ones((lidar_hit_num, 1))
        )

        hit_mask = lidar_count.squeeze(1) > 0
        lidar_merged[hit_mask] = lidar_sum[hit_mask] / lidar_count[hit_mask]
        return torch.cat([lidar_merged, image_output], dim=0)
    ...
    def forward(self, batch_dict, agent=None):
        ...
        if self.lidar2image_on: # 将lidar的feature映射到image的feature 将激光雷达数据预处理为图像特征，以增强图像的特征表示
            if self.use_mamba_inter2: #True
                ...
                lidar2image_relation_features = None
                if self.use_multi_scale: #False
                    lidar2image_outputs = self._lidar2image_preprocess(
                        batch_dict, multi_feat, multi_pos_embed_list)
                    if self.use_lidar2image_relation:
                        lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, lidar2image_coor, lidar2image_coords_bzyx_list, voxel_hit_indices, lidar2image_relation_features = lidar2image_outputs
                    else:
                        lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, lidar2image_coor, lidar2image_coords_bzyx_list, voxel_hit_indices = lidar2image_outputs
                    ...
                    lidar2image_outputs = self._lidar2image_preprocess(batch_dict, multi_feat, multi_pos_embed_list)
                    if self.use_lidar2image_relation:
                        lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, lidar2image_coor, voxel_hit_indices, lidar2image_relation_features = lidar2image_outputs
                    else:
                        lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, lidar2image_coor, voxel_hit_indices = lidar2image_outputs
                ...
                if self.win_version == 'v4':   #False
                    ...
                    if self.shape_inter2[0] == 2:
                        lidar2image_coor[:voxel_hit_indices.shape[0], 1] = 1
            else:
                lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, voxel_hit_indices = self._lidar2image_preprocess(
                    batch_dict, multi_feat, multi_pos_embed_list)
                        ...
                        if self.use_mixed:
                            ...
                            if i == self.lidar2image_start:
                                if self.mask_out_img:
                                    ...
                                    lidar2image_lidar_num = voxel_hit_indices.shape[0]
                                    expanded_lidar_output = output[:voxel_num][voxel_hit_indices]
                                    if self.use_lidar2image_relation:
                                        if lidar2image_relation_features is None:
                                            raise ValueError("lidar2image relation features are required when USE_LIDAR2IMAGE_RELATION=True")
                                        if lidar2image_relation_features.shape[0] != lidar2image_lidar_num:
                                            raise ValueError(
                                                "lidar2image relation features are not aligned with projected voxels: "
                                                f"relation={lidar2image_relation_features.shape[0]}, projected={lidar2image_lidar_num}"
                                            )
                                        relation_embed = self.lidar2image_relation_mlp(
                                            lidar2image_relation_features.to(
                                                device=expanded_lidar_output.device,
                                                dtype=expanded_lidar_output.dtype,
                                            )
                                        )
                                        expanded_lidar_output = expanded_lidar_output + self.lidar2image_relation_scale * relation_embed
                                    output_lidar2image = torch.cat([expanded_lidar_output, output[voxel_num:]], dim=0)
                                    output_sparse = spconv.SparseConvTensor(
                                        features=output_lidar2image,
                                        indices=lidar2image_coor.int(),
                                        spatial_shape=self.shape_inter2,
                                        batch_size=batch_dict['batch_size'] * lidar2image_view_num
                                    )
                                ...
                                if self.mask_out_img:
                                    ...
                                    output_sparse = replace_feature(output_sparse, output_lidar2image)
                            ...
                            if self.use_fixed_mapping:
                                ...
                                if self.mask_out_img:
                                    ...
                                    output_lidar2image, output_sparse = self.forward_stage2(
                                        block,
                                        output_sparse,
                                        lidar2image_coor.int(),
                                        batch_dict,
                                        i,
                                        voxel_num=lidar2image_lidar_num
                                    )
                                    output = self._merge_lidar2image_multi_view_output(
                                        output_lidar2image,
                                        residual,
                                        voxel_hit_indices,
                                        voxel_num
    ...
    def _lidar2image_preprocess(self, batch_dict, multi_feat, multi_pos_embed_list):
        ...
        if self.use_lidar2image_relation:
            lidar2image_coords_zyx, lidar2image_coords_bzyx_list, voxel_hit_indices, lidar2image_relation_features = self.map_lidar2image_layer(
                batch_dict, self.use_multi_scale, return_relation=True)
        else:
            lidar2image_coords_zyx, lidar2image_coords_bzyx_list, voxel_hit_indices = self.map_lidar2image_layer(
                batch_dict, self.use_multi_scale)
            lidar2image_relation_features = None
        lidar2image_coords_bzyx = torch.cat(
            [batch_dict['voxel_coords'][voxel_hit_indices, :1].clone(), lidar2image_coords_zyx], dim=1) # torch.Size([num_hits, 4])
       ...
       # self._visualize_lidar2image_projection(batch_dict, lidar2image_coords_zyx, lidar2image_coords_bzyx,outer_dict)
        ...
        if not (self.use_mamba_inter2 and not self.use_mixed):
            ...
            expanded_lidar_feat = multi_feat[:voxel_num][voxel_hit_indices]
            lidar2image_batch_dict['voxel_features'] = torch.cat(
                [expanded_lidar_feat, multi_feat[voxel_num:]], dim=0)
            lidar2image_batch_dict['voxel_coords'] = torch.cat(
                [lidar2image_coords_bzyx, multiview_coords], dim=0) # [num_hits + num_of_patch, 4] batch_id view_id x(width) y(height)
        ...
        if self.use_mamba_inter2:   #True
            ...
            if self.use_mixed:
                if self.use_lidar2image_relation:
                    return lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, final_coords, voxel_hit_indices, lidar2image_relation_features
                return lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, final_coords, voxel_hit_indices
            elif self.use_multi_scale:
                if self.use_lidar2image_relation:
                    return None, None, multi_pos_embed_list, final_coords, lidar2image_coords_bzyx_list, voxel_hit_indices, lidar2image_relation_features
                return None, None, multi_pos_embed_list, final_coords, lidar2image_coords_bzyx_list, voxel_hit_indices
            else:
                if self.use_lidar2image_relation:
                    return None, None, multi_pos_embed_list, final_coords, voxel_hit_indices, lidar2image_relation_features
                return None, None, multi_pos_embed_list, final_coords, voxel_hit_indices
        else:
            return lidar2image_inds_list, lidar2image_masks_list, multi_pos_embed_list, voxel_hit_indices
