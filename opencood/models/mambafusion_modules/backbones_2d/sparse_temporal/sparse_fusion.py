...
class TemporalNeedHead(nn.Module):
    """Predict temporal confidence and need maps from fused BEV plus fusion cues."""
    ...
    def __init__(
        ...
        Args:
            ...
            smoothing_kernel_size: Average smoothing kernel for confidence maps.
        """
        super().__init__()
        ...
        aux_channels = 2 * (self.cue_distance_channels + 1)
    ...
    def _build_source_aux(
        ...
        return torch.cat([normalized_distance, normalized_gate], dim=1)
    ...
    def forward(
        ...
        """Predict current-frame confidence then convert it into temporal need.
        ...
        aux_drone = self._build_source_aux(
            distance_drone,
            gate_drone,
            batch_size,
            height,
        ...
        aux_rsu = self._build_source_aux(
            distance_rsu,
            gate_rsu,
            batch_size,
            height,
        ...
        confidence_logits = self.confidence_head(torch.cat([current_feature, aux_drone, aux_rsu], dim=1))
        confidence_map = torch.sigmoid(confidence_logits)

        if self.smoothing_kernel_size > 1:
            kernel = self.smoothing_kernel_size
            padding = kernel // 2
            smoothed_confidence = F.avg_pool2d(
                confidence_map,
                kernel_size=kernel,
                stride=1,
                padding=padding,
            ...
            smoothed_confidence = confidence_map
        need_map = 1.0 - smoothed_confidence
