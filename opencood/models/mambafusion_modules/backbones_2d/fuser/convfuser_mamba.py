...
class SampleAttentionAggregator(nn.Module):
    ...
    def forward(
        ...
        if query_feature is None:
            query_feature = sampled_features.mean(dim=1)
        ...
        attention_logits = self.attention_head(self.query_norm(query_feature))
        ...
        refined_feature = residual_query + self.layer_scale_attn * self.output_proj(aggregated_feature)
...
class OffsetGuidedSelectiveHierarchicalMambaFusionBlock(nn.Module):
    ...
    def __init__(
        ...
        cue_channels = 4 * self.total_num_points + self.semantic_channels
        self.semantic_heads = nn.ModuleDict(
            {
                'rsu': SemanticHead(channels),
                'drone': SemanticHead(channels),
            }
        )
        self.final_gate_heads = nn.ModuleDict(
            {
                'rsu': CueBasedFinalGateHead(
                    in_channels=cue_channels,
    ...
    def _build_final_gate(
        ...
        cue_dict: Dict[str, torch.Tensor],
    ) -> torch.Tensor:
        """Build the final cue-based gate for one external source."""
        semantic = self.semantic_heads[source_name](refined_feature)  # [B, 4, H, W]
        ...
        return self.final_gate_heads[source_name](gate_input)  # [B, 1, H, W]
    ...
    def _process_optional_external_source(
        ...
        final_gate = self._build_final_gate(
    ...
    def forward(
        ...
        if self.sparse_temporal_fusion is not None:
            ...
            fused_feature = self.sparse_temporal_fusion(
        ...
        local_feature = self.local_mamba(fused_feature, return_debug=False)
