...
def resize_crop_box2d(box,resize,crop,h,w):
    x1,y1,x2,y2,k = box
    crop_w, crop_h = crop[:2]
    x1 = round(x1 * resize - crop_w)
    x2 = round(x2 * resize - crop_w)
    y1 = round(y1 * resize - crop_h)
    y2 = round(y2 * resize - crop_h)
    resize_hw = (y2-y1,x2-x1)
    x1 = min(w,max(0,x1))
    y1 = min(h,max(0,y1))
    x2 = max(0,min(x2,w))
    y2 = max(0,min(y2,h))
    trunc_box2d = (x1,y1,x2,y2,k)
    return trunc_box2d, resize_hw
