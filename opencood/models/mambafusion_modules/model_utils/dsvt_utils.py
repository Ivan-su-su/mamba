...
def get_window_coors(coors, sparse_shape, window_shape, do_shift, shift_list=None, return_win_coors=False):
    ...
    if len(window_shape) == 2:
        ...
        win_shape_x, win_shape_y, win_shape_z = window_shape # [30, 30, 1] [30, 30, 1]

    sparse_shape_x, sparse_shape_y, sparse_shape_z = sparse_shape # [32, 88, 1] [360, 360, 1] 这里的32和88是指的输入的特征图的大小
    assert sparse_shape_z < sparse_shape_x, 'Usually holds... in case of wrong order'

    max_num_win_x = int(np.ceil((sparse_shape_x / win_shape_x)) + 1) # plus one here to meet the needs of shift. 3 13
    max_num_win_y = int(np.ceil((sparse_shape_y / win_shape_y)) + 1) # plus one here to meet the needs of shift. 4 13
    max_num_win_z = int(np.ceil((sparse_shape_z / win_shape_z)) + 1) # plus one here to meet the needs of shift. 2 2
    max_num_win_per_sample = max_num_win_x * max_num_win_y * max_num_win_z # 24

    if do_shift:
        if shift_list is not None:
            shift_x, shift_y, shift_z = shift_list[0], shift_list[1], shift_list[2] # [15, 15, 0]
        else:
    ...
    win_coors_x = shifted_coors_x // win_shape_x # 得到每个点所在的window的坐标
    ...
    batch_win_inds = coors[:, 0] * max_num_win_per_sample + \
                        ...
                        win_coors_z # 得到每个点所在的window的index

    coors_in_win_x = shifted_coors_x % win_shape_x # 得到每个点在window中的坐标
