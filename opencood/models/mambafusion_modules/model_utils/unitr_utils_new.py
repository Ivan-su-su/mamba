...
class MapLidar2Image(nn.Module):
    ...
    def map_lidar2image_multi_view(self, points, lidar2image, image_aug_matrix, batch_idx, image_shape, return_relation=False):
        """Map lidar voxels to every valid camera view.

        Returns:
            batch_hit_points: [num_hits, 3], each row is [u, v, view_idx].
            point_ids: [num_hits], linear point ids inside ``points``.
            relation_features: optional [num_hits, 9] geometry relation encoding.
        """
        points_batch = points.float()
        lidar2image_batch = lidar2image.float()
        image_aug_matrix_batch = image_aug_matrix.float()
        ...
        if hit_indices.numel() == 0:
            ...
            if return_relation:
                empty_relation = points_batch.new_empty((0, 9))
                return empty_hit_points, empty_point_ids, empty_relation
        ...
        if return_relation:
            relation_features = self._build_lidar2image_relation_features(
                points_batch,
                lidar2image_batch,
                hit_coords,
                view_ids,
            ...
            return hit_points, point_ids.long(), relation_features
        return hit_points, point_ids.long()

    def _build_lidar2image_relation_features(
        self,
        points_batch,
        lidar2image_batch,
        hit_coords,
        ...
        """Build per projected-voxel geometry features for lidar2image fusion."""
        hw_h = max(int(image_shape[0] // 8), 1)
        hw_w = max(int(image_shape[1] // 8), 1)
        grid_scale = hit_coords.new_tensor([hw_w, hw_h])
        grid_xy = hit_coords * grid_scale
        cell_center_xy = torch.floor(grid_xy) + 0.5
        duv = grid_xy - cell_center_xy

        selected_points = points_batch[grid_ids, sample_ids]
        selected_points_h = torch.cat(
            [selected_points, torch.ones_like(selected_points[:, :1])],
            dim=-1
        )
        selected_lidar2image = lidar2image_batch[0, view_ids]
        proj_h = torch.bmm(
            selected_lidar2image,
            selected_points_h.unsqueeze(-1)
        ).squeeze(-1)

        eps = 1e-5
        depth = proj_h[:, 2].clamp_min(eps)
        # 二选一即可
        depth_norm = depth / 100.0
        log_depth_norm = torch.log1p(depth) / torch.log1p(depth.new_tensor(100.0))
    ...
    def map_lidar2image_instance_views(
        self,
        points,
        lidar2image,
        image_aug_matrix,
        ...
        """Map each instance's voxels only to that instance's camera views."""
        if instance_voxel_mask.ndim != 2:
            raise ValueError(
                f"instance_voxel_mask must be [num_instances, num_voxels], got {tuple(instance_voxel_mask.shape)}"
            )
        if instance_voxel_mask.shape[1] != points.shape[0]:
            raise ValueError(
                "instance_voxel_mask is not aligned with lidar voxels: "
                f"mask has {instance_voxel_mask.shape[1]} columns but points has {points.shape[0]} rows"
            )

        num_instances = int(instance_voxel_mask.shape[0])
        total_views = int(lidar2image.shape[1])
        if num_instances <= 0:
            empty_hit_points = points.new_empty((0, 3))
            empty_voxel_ids = torch.empty((0,), device=points.device, dtype=torch.long)
            if return_relation:
                return empty_hit_points, empty_voxel_ids, points.new_empty((0, 9))
            return empty_hit_points, empty_voxel_ids
        if total_views % num_instances != 0:
            raise ValueError(
                "Cannot split camera views evenly by instance: "
                f"total_views={total_views}, num_instances={num_instances}"
            )

        views_per_instance = total_views // num_instances
        instance_voxel_mask = instance_voxel_mask.bool().to(points.device)
        all_hit_points = []
        all_voxel_hit_indices = []
        all_relation_features = []
        sample_num = points.shape[1]

        for inst_idx in range(num_instances):
            voxel_ids = torch.nonzero(instance_voxel_mask[inst_idx], as_tuple=False).squeeze(-1)
            if voxel_ids.numel() == 0:
                continue

            view_start = inst_idx * views_per_instance
            view_end = view_start + views_per_instance
            map_outputs = self.map_lidar2image_multi_view(
                points[voxel_ids],
                lidar2image[:, view_start:view_end],
                image_aug_matrix[:, view_start:view_end],
                batch_idx[voxel_ids],
                image_shape,
                return_relation=return_relation,
            )
            if return_relation:
                hit_points_i, local_point_ids, relation_features_i = map_outputs
            else:
                hit_points_i, local_point_ids = map_outputs
            if hit_points_i.shape[0] == 0:
                continue

            local_voxel_ids = torch.div(local_point_ids, sample_num, rounding_mode='floor')
            hit_points_i[:, 2] += view_start
            if return_relation:
                view_denom = max(total_views - 1, 1)
                relation_features_i[:, -1] = hit_points_i[:, 2] / float(view_denom)
                all_relation_features.append(relation_features_i)
            all_hit_points.append(hit_points_i)
            all_voxel_hit_indices.append(voxel_ids[local_voxel_ids])

        if len(all_hit_points) == 0:
            empty_hit_points = points.new_empty((0, 3))
            empty_voxel_ids = torch.empty((0,), device=points.device, dtype=torch.long)
            if return_relation:
                return empty_hit_points, empty_voxel_ids, points.new_empty((0, 9))
        ...
        if return_relation:
            return (
                torch.cat(all_hit_points, dim=0),
                torch.cat(all_voxel_hit_indices, dim=0).long(),
                torch.cat(all_relation_features, dim=0),
            )
        return torch.cat(all_hit_points, dim=0), torch.cat(all_voxel_hit_indices, dim=0).long()

    def forward(self, batch_dict, use_multi_scale=False, space_shape=None, return_relation=False):
        ...
        with torch.no_grad():
            ...
            instance_voxel_mask = batch_dict.get('instance_voxel_mask')
            if instance_voxel_mask is not None:
                map_outputs = self.map_lidar2image_instance_views(
                    points,
                    lidar2image,
                    img_aug_matrix,
                    batch_idx,
                ...
                if return_relation:
                    lidar2image_coords_xyz, voxel_hit_indices, relation_features = map_outputs
                else:
                    lidar2image_coords_xyz, voxel_hit_indices = map_outputs
            else:
                lidar2image_coords_xyz = self.map_lidar2image(
                    points, lidar2image, img_aug_matrix, batch_idx, img_shape, coords)
                voxel_hit_indices = torch.arange(
                    points.shape[0], device=coords.device, dtype=torch.long)
                if return_relation:
                    relation_features = lidar2image_coords_xyz.new_zeros((lidar2image_coords_xyz.shape[0], 9))
        ...
        if use_multi_scale: 
            ...
            if return_relation:
                return lidar2image_coords_zyx, lidar2image_coords_zyx_list, voxel_hit_indices, relation_features
            return lidar2image_coords_zyx, lidar2image_coords_zyx_list, voxel_hit_indices
        
        if return_relation:
            return lidar2image_coords_zyx, None, voxel_hit_indices, relation_features
