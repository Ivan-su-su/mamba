...
def get_hilbert_index_3d_mamba_lite(template, coors, batch_size, z_dim, hilbert_spatial_size, shift=(0, 0, 0), debug=True):
    ...
    for i in range(batch_size):
        ...
        inds_curt_to_next[i] = torch.argsort(hil_inds[batch_mask]) # sort the indices of hil_inds
        inds_next_to_curt[i] = torch.argsort(inds_curt_to_next[i]) # sort the indices of inds_curt_to_next
