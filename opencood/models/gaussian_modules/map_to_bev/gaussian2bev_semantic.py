...
class ContinuousGaussianVFE(nn.Module):
    ...
    @torch.no_grad()
    def _discrete_voxel_indices(self, mu: torch.Tensor, bidx: torch.Tensor, device: torch.device):
        """
        连续坐标 → 整数体素索引 (b,z,y,x) 以及有效掩码
        mu: [N,3] in (x,y,z) world
        """
        self.pc_min = self.pc_min.to(device)
        self.pc_max = self.pc_max.to(device)
    ...
    neighbor_info: dict = None
        ...
        if neighbor_info is not None:
            ...
            if idxs is not None and masks is not None and idxs.numel() > 0:
                ...
                out_semantic = torch.einsum('mk,mkc->mc', w, semantic_nb)  # [M,C]
...
class BEVScatterWithHeight(nn.Module):
    ...
    def __init__(
        ...
        if fuse_mode == 'add' and height_embed_dim != (feature_dim+4):
            self.proj_height_to_feat = nn.Conv2d(height_embed_dim, (feature_dim+4), kernel_size=1, bias=True)
        else:
...
class GaussianToBEV(nn.Module):
    ...
    def __init__(
        ...
        self.voxel_size = model_cfg.get('VOXEL_SIZE', [0.25, 0.25, 2])
        ...
        self.output_feature_dim = model_cfg.get('OUTPUT_FEATURE_DIM', 128)
        self.use_neighbor_association = model_cfg.get('USE_NEIGHBOR_ASSOCIATION', True)
        self.use_refinement = model_cfg.get('USE_REFINEMENT', True)
        self.aggregation = model_cfg.get('AGGREGATION', 'mean')
        self.quat_pooling = model_cfg.get('QUAT_POOLING', 'antipodal')
        self.return_abs_height = model_cfg.get('RETURN_ABS_HEIGHT', True)
        self.use_mahalanobis_distance = model_cfg.get('USE_MAHALANOBIS_DISTANCE', True)
        self.temperature = model_cfg.get('TEMPERATURE', 1.0)
        self.normalize_weights = model_cfg.get('NORMALIZE_WEIGHTS', True)
        self.chunk = model_cfg.get('CHUNK', 4096)
        self.scale_multiplier = model_cfg.get('SCALE_MULTIPLIER', 3.0)
        self.max_neighbors = model_cfg.get('MAX_NEIGHBORS', 64)
        self.assoc_chunk = model_cfg.get('ASSOC_CHUNK', 4096)
        self.aggregation_method = model_cfg.get('AGGREGATION_METHOD', 'mean')
        ...
        self.vfe = ContinuousGaussianVFE(
            ...
            quat_pooling=self.quat_pooling,
            return_abs_height=self.return_abs_height
        )

        # ---------------- 2) 高斯混合累加器 ----------------
        self.mixture_accumulator = GaussianMixtureAccumulator(
            feature_dim=self.feature_dim,
            use_mahalanobis_distance=self.use_mahalanobis_distance,
        ...
        self.bev_scatter = BEVScatterWithHeight(
            ...
            z_bins=self.z_bins,
            height_embed_dim=self.height_embed_dim,
            fuse_mode=self.fuse_mode,  # 'concat' | 'add'
        )
    ...
    def forward(self, batch_dict: Optional[Dict] = None, available_agents: List[str] = None):
        """
        Returns:
            bev_features: [B, C_out, H, W]
            intermediates: Dict
