...
class GaussianTPVRefiner(nn.Module):
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        use_voxel_knn: bool = True,
        voxel_size: List[float] = [0.4, 0.4, 0.125],
        **kwargs
    ):
        super().__init__()
        ...
        self.use_voxel_knn = use_voxel_knn
        ...
        self.self_attention = SparseGaussianSelfAttention(
            ...
            use_voxel_knn=self.use_voxel_knn
        )
        ...
        self.cross_attention = GaussianTPVCrossAttention(
            ...
            use_offset=True,
            fuse='concat',  # 或者 'sum'
            num_anchors=5 + num_learnable_pts
        )
    ...
    def forward(
        ...
        img_query_gaussians: Optional[Dict[str, torch.Tensor]] = None,
        lidar_query_gaussians: Optional[Dict[str, torch.Tensor]] = None,
        query_gaussians: Optional[Dict[str, torch.Tensor]] = None,
        ...
        if merged_gaussians is None:
            merged_gaussians, merged_query_gaussians = self.gaussian_aggregator(
            ...
            merged_gaussians, merged_query_gaussians = self.gaussian_aggregator(
                query_gaussians=query_gaussians,
                merged_gaussians=merged_gaussians,
                tpv_spatial_shapes=spatial_shapes  # 传入spatial_shapes进行归一化
            )  # Dict: 'mu', 'scale', 'rotation', 'features', 'ref_xy', 'ref_xz', 'ref_yz' (已归一化)
        
        # === 基于 mu 计算 voxel_coords，供 voxel-KNN 使用 ===
        xyz,xyz_query = merged_gaussians['mu'], merged_query_gaussians['mu']  # [N, 3]
        if xyz.numel() == 0:
            voxel_coords = torch.empty(0, 3, dtype=torch.long, device=xyz.device)
            voxel_coords_query = torch.empty(0, 3, dtype=torch.long, device=xyz_query.device)
        else:
            pc_min = self.pc_range[:3].to(xyz.device)  # (x_min, y_min, z_min)
            voxel_size = self.voxel_size.to(xyz.device)  # (vx, vy, vz)
            rel = xyz - pc_min.unsqueeze(0)
            rel_query = xyz_query - pc_min.unsqueeze(0)
            voxel_coords = torch.floor(rel / voxel_size.unsqueeze(0)).long()
            voxel_coords_query = torch.floor(rel_query / voxel_size.unsqueeze(0)).long()
        merged_gaussians['voxel_coords'] = voxel_coords
        merged_query_gaussians['voxel_coords'] = voxel_coords_query
        
        # TPV特征投影到embed_dims维度
        tpv_value = self.tpv_proj(tpv_value)  # [B, sum(H_l*W_l), embed_dims]
        
        # 3. 高斯点自注意力（k-NN 稀疏版）
        self_attn_features,gaussian_features_query = self.self_attention(
            features=merged_gaussians['features'],  # [N, embed_dims]
            means=merged_gaussians['mu'],  # [N, 3]
            voxel_coords=merged_gaussians['voxel_coords'],  # [N, 3]
            query_features=merged_query_gaussians['features'],  # [N, embed_dims]
            query_means=merged_query_gaussians['mu'],  # [N, 3]
            query_voxel_coords=merged_query_gaussians['voxel_coords']  # [N, 3]
        )  # [N, embed_dims]
        ...
        merged_query_gaussians['features'] = gaussian_features_query
        # 6. 解码参数并更新高斯点
        updated_gaussians,updated_query_gaussians = self.gaussian_decoder(
            original_gaussian = merged_gaussians, # Dict
            query_gaussian = merged_query_gaussians  # Dict
        )  # Dict: 'mu', 'scale', 'rotation', 'features'
        
        return updated_gaussians,updated_query_gaussians
...
class GaussianAggregator(nn.Module):
    ...
    def __init__(self, img_feature_dim: int, lidar_feature_dim: int, output_dim: int, pc_range: Tuple[float, float, float, float, float, float], num_learnable_pts: int = 0):
        ...
        # 几何embedding MLP: (mu(3) + scale(3) + rotation(4) + semantic(4)) = 14维 → output_dim
    ...
    def forward(
        ...
        if query_gaussians is not None:
            means_query = query_gaussians['mu']  # [N, 3]
            scales_query = query_gaussians['scale']  # [N, 3]
            rotations_query = query_gaussians['rotation']  # [N, 4]
            Q_query = query_gaussians['features']  # [N, D]
            semantic_query = query_gaussians['semantic']  # [N, 4]
            assert img_query_gaussians is not None and lidar_query_gaussians is not None, \
                "Either (img_query_gaussians, lidar_query_gaussians) or query_gaussians must be provided"
        else:
            assert img_query_gaussians is not None and lidar_query_gaussians is not None, \
                "Either (img_query_gaussians, lidar_query_gaussians) or query_gaussians must be provided"
            img_query_feat = self.img_align(img_query_gaussians['features'])  # [Ni, D]
            lidar_query_feat = self.lidar_align(lidar_query_gaussians['features'])  # [Nl, D]
            Q_query = torch.cat([img_query_feat, lidar_query_feat], dim=0)   # [N,D] 原始特征
            means_query = torch.cat([img_query_gaussians['mu'], lidar_query_gaussians['mu']], dim=0)              # [N,3]
            scales_query = torch.cat([img_query_gaussians['scale'], lidar_query_gaussians['scale']], dim=0)        # [N,3]
            rotations_query = torch.cat([img_query_gaussians['rotation'], lidar_query_gaussians['rotation']], dim=0)  # [N,4]
            semantic_query = torch.cat([img_query_gaussians['semantic'], lidar_query_gaussians['semantic']], dim=0)  # [N, 4]
        
        # 3) 构建几何embedding E: [N, 10] = [mu(3) + scale(3) + rotation(4)]
        geometry_vec = torch.cat([means, scales, rotations, semantic], dim=-1)  # [N, 10]
        E = self.geometry_embedding(geometry_vec)  # [N, output_dim]
        geometry_vec_query = torch.cat([means_query, scales_query, rotations_query, semantic_query], dim=-1)  # [N, 14]
        E_query = self.geometry_embedding(geometry_vec_query) # [N, output_dim]
        # 4) 更新特征: features = E + Q
        features = E + Q  # [N, output_dim]
        features_query = E_query + Q_query  # [N, output_dim]
        ...
        merged_query = {
            'mu': means_query,
            'scale': scales_query,
            'rotation': rotations_query,
            'features': features_query, #可能要投影到同一语义空间？或者通过一个mlp+residual TODO
        }
...
class SparseGaussianSelfAttention(nn.Module):
    ...
    - normal gaussians: G–G 自注意力
    - query gaussians:  Q–G 交叉注意力（只看 normal，不看 query 自己）
    """
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        max_distance: float = 10.0,
        use_voxel_knn: bool = False,
        radius_levels: Optional[List[int]] = None,
        neighbors_per_radius: Optional[List[int]] = None,
    ):
        super().__init__()
        ...
        # ====== voxel ring 配置 ======
        if self.use_voxel_knn:
            if radius_levels is None:
                radius_levels = [0, 1, 2, 3]
            if neighbors_per_radius is None:
                neighbors_per_radius = [1, 10, 6, 4]
            assert len(radius_levels) == len(neighbors_per_radius), \
                "radius_levels 与 neighbors_per_radius 长度必须相同"
            self.radius_levels = radius_levels
            self.neighbors_per_radius = neighbors_per_radius
            self.max_radius = max(self.radius_levels) if self.radius_levels else 0
            self.shell_offsets = self._precompute_shell_offsets()
        else:
            self.radius_levels = []
            self.neighbors_per_radius = []
            self.max_radius = 0
            self.shell_offsets = {}
        
        # QKV投影层（对 normal 和 query 共用）
    ...
    def _precompute_shell_offsets(self) -> Dict[int, List[Tuple[int, int, int]]]:
        """
        按 Chebyshev 距离预计算 ring shell：
        shell(r) = {(dx,dy,dz) | max(|dx|,|dy|,|dz|) == r}
        """
        shell_offsets: Dict[int, List[Tuple[int, int, int]]] = {}
        for r in self.radius_levels:
            offsets_r: List[Tuple[int, int, int]] = []
            for dx in range(-r, r + 1):
                for dy in range(-r, r + 1):
                    for dz in range(-r, r + 1):
                        if max(abs(dx), abs(dy), abs(dz)) == r:
                            offsets_r.append((dx, dy, dz))
            shell_offsets[r] = offsets_r
    ...
    def build_knn_graph(self, means: torch.Tensor):
        ...
        N = means.shape[0]
        if N == 0:
            empty_idx = torch.zeros((0, 0), dtype=torch.long, device=means.device)
            empty_mask = torch.zeros((0, 0), dtype=torch.bool, device=means.device)
    ...
    def build_voxel_knn_graph(
        self,
        means: torch.Tensor,
        voxel_coords: torch.Tensor,
    ):
        """
        使用 voxel ring 邻居构建 k-NN 图（多半径 ring + L2 距离，距离/TopK 在 GPU 上计算）
        """
        device = means.device
        N = means.shape[0]
        
        if N == 0:
            empty_idx = torch.zeros((0, 0), dtype=torch.long, device=device)
            empty_mask = torch.zeros((0, 0), dtype=torch.bool, device=device)
            return empty_idx, empty_mask
        
        max_k = min(self.k_neighbors + 1, max(1, N))
        
        vox_cpu = voxel_coords.detach().cpu().to(torch.int32)
        voxel_dict: Dict[Tuple[int, int, int], List[int]] = defaultdict(list)
        for idx, v in enumerate(vox_cpu):
            key = (int(v[0]), int(v[1]), int(v[2]))
            voxel_dict[key].append(idx)
        
        knn_indices = torch.zeros((N, max_k), dtype=torch.long, device=device)
        valid_mask = torch.zeros((N, max_k), dtype=torch.bool, device=device)
        
        max_dist2 = (self.max_distance ** 2) if self.max_distance is not None else None
        means_gpu = means
        
        for i in range(N):
            vx_i, vy_i, vz_i = int(vox_cpu[i, 0]), int(vox_cpu[i, 1]), int(vox_cpu[i, 2])
            neighbors: List[int] = [i]
            
            for r, max_nbr in zip(self.radius_levels, self.neighbors_per_radius):
                if len(neighbors) >= max_k:
                    break
                
                offsets_r = self.shell_offsets.get(r, [])
                shell_candidates: List[int] = []
                for dx, dy, dz in offsets_r:
                    key = (vx_i + dx, vy_i + dy, vz_i + dz)
                    if key in voxel_dict:
                        shell_candidates.extend(voxel_dict[key])
                
                if not shell_candidates:
                    continue
                shell_candidates = list(set(shell_candidates) - set(neighbors))
                if not shell_candidates:
                    continue
                
                cand_idx = torch.tensor(shell_candidates, device=device, dtype=torch.long)
                center = means_gpu[i].unsqueeze(0)
                cand_xyz = means_gpu[cand_idx]
                dist2 = ((cand_xyz - center) ** 2).sum(-1)
                ...
                k_here = min(max_nbr, cand_idx.numel())
                if k_here <= 0:
                    continue
                
                _, topk_pos = torch.topk(dist2, k_here, largest=False)
                selected = cand_idx[topk_pos]
                
                neighbors.extend(selected.tolist())
                if len(neighbors) >= max_k:
                    break
            
            neighbors = neighbors[:max_k]
            m = len(neighbors)
            if m > 0:
                knn_indices[i, :m] = torch.tensor(neighbors, device=device, dtype=torch.long)
                valid_mask[i, :m] = True
        
        empty_rows = ~valid_mask.any(dim=1)
        if empty_rows.any():
            self_indices = torch.arange(N, device=device)
            knn_indices[empty_rows, 0] = self_indices[empty_rows]
            valid_mask[empty_rows, 0] = True
        
        return knn_indices, valid_mask

    # ============== NEW: query → gauss 的 cross kNN ==================
    def build_cross_knn_graph(
        self,
        query_means: torch.Tensor,   # [Nq, 3]
        key_means: torch.Tensor,     # [Ng, 3]
    ):
        """
        query_means 对 key_means 做 kNN（用 FAISS；比 voxel 简单稳定）
        返回:
            knn_indices: [Nq, k] (index into key_means)
            valid_mask:  [Nq, k]
        """
        device = query_means.device
        Nq = query_means.shape[0]
        Ng = key_means.shape[0]
        if Nq == 0 or Ng == 0:
            empty_idx = torch.zeros((Nq, 0), dtype=torch.long, device=device)
            empty_mask = torch.zeros((Nq, 0), dtype=torch.bool, device=device)
            return empty_idx, empty_mask
        
        max_k = min(self.k_neighbors, Ng)

        key_np = key_means.detach().cpu().numpy().astype('float32')
        qry_np = query_means.detach().cpu().numpy().astype('float32')

        index = faiss.IndexFlatL2(3)
        index.add(key_np)
        dist, idx = index.search(qry_np, max_k)

        knn_indices = torch.from_numpy(idx).to(device=device, dtype=torch.long)  # [Nq, k]
        dist = torch.from_numpy(dist).to(device=device)

        valid_mask = dist <= (self.max_distance ** 2)
    ...
    def gather_neighbors(self, x: torch.Tensor, knn_idx: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            x: [Ng, H, D] tensor of key features
            knn_idx: [Nq, k] tensor of neighbor indices (into dim=0 of x)
        
        Returns:
            neighbors: [Nq, H, k, D] tensor of gathered neighbors
        """
        Ng, H, D = x.shape
        Nq, k = knn_idx.shape
        
        # Use reshape instead of view to avoid non-contiguous issues
        x_flat = x.reshape(Ng, H * D)      # [Ng, H*D]
        neighbors_flat = x_flat[knn_idx]   # [Nq, k, H*D]
        neighbors = neighbors_flat.reshape(Nq, k, H, D).permute(0, 2, 1, 3).contiguous()  # [Nq, H, k, D]
    ...
    def forward(
        ...
        query_means: Optional[torch.Tensor] = None,
        query_voxel_coords: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor]]:
        """
        稀疏注意力:
        - normal gaussians: G–G 自注意力
        - query gaussians:  Q–G 交叉注意力

        Args:
            features: [Ng, C] 正常高斯特征 (K/V 的来源, 也做自己的 Q)
            means:    [Ng, 3]
            voxel_coords: [Ng, 3]
            query_features: [Nq, C]  (只作为 Q，不进 K/V)
            query_means:    [Nq, 3]
            query_voxel_coords: [Nq, 3] (目前没用到，预留接口)

        Returns:
            gauss_out: [Ng, C]
            query_out: [Nq, C] or None
        """
        Ng, D = features.shape
        if Ng == 0:
            # 没有 normal gauss，直接返回
            return features, query_features

        # =================== 1) normal gauss: G–G self-attn ===================
        # 1. 构建k-NN图（带距离限制）
        if self.use_voxel_knn:
            assert voxel_coords is not None, "use_voxel_knn=True 时必须提供 voxel_coords"
            knn_indices_g, valid_mask_g = self.build_voxel_knn_graph(
                means=means, voxel_coords=voxel_coords
            )
        else:
            knn_indices_g, valid_mask_g = self.build_knn_graph(means)
        kg = knn_indices_g.shape[1]
        
        # 2. QKV投影 (normal gauss)
        Q_g = self.q_proj(features).contiguous().reshape(Ng, self.num_heads, self.head_dims)
        K_g = self.k_proj(features).contiguous().reshape(Ng, self.num_heads, self.head_dims)
        V_g = self.v_proj(features).contiguous().reshape(Ng, self.num_heads, self.head_dims)
        
        # 3. 采样邻居的K和V
        K_neighbors_g = self.gather_neighbors(K_g, knn_indices_g)  # [Ng, H, kg, D]
        V_neighbors_g = self.gather_neighbors(V_g, knn_indices_g)  # [Ng, H, kg, D]
        
        # 4. 计算attention logits (scale → mask → softmax)
        Q_expanded_g = Q_g.unsqueeze(2)  # [Ng, H, 1, D]
        
        # Compute raw logits
        attn_logits_g = torch.bmm(
            Q_expanded_g.reshape(Ng * self.num_heads, 1, self.head_dims),
            K_neighbors_g.reshape(Ng * self.num_heads, kg, self.head_dims).transpose(1, 2)
        ).reshape(Ng, self.num_heads, 1, kg)
        
        # Scale
        attn_logits_g = attn_logits_g / (self.head_dims ** 0.5)
        
        # Mask invalid neighbors
        valid_mask_expanded_g = valid_mask_g.unsqueeze(1).unsqueeze(2)  # [Ng, 1, 1, kg]
        NEG_INF = torch.finfo(attn_logits_g.dtype).min / 2 if attn_logits_g.dtype in [torch.float16, torch.bfloat16] else -1e9
        attn_logits_g = attn_logits_g.masked_fill(~valid_mask_expanded_g, NEG_INF)
        
        # Softmax
        attn_weights_g = F.softmax(attn_logits_g, dim=-1)
        attn_weights_g = self.dropout(attn_weights_g)
        
        # 5. 加权求和
        output_g = torch.bmm(
            attn_weights_g.reshape(Ng * self.num_heads, 1, kg), 
            V_neighbors_g.reshape(Ng * self.num_heads, kg, self.head_dims)
        ).reshape(Ng, self.num_heads, self.head_dims)
        
        # 6. 拼接多头
        output_g = output_g.contiguous().reshape(Ng, self.num_heads * self.head_dims)  # [Ng, embed_dims]
        
        # 7. 输出投影 + 残差 + LN
        output_g = self.output_proj(output_g)
        output_g = self.dropout(output_g)
        gauss_out = self.norm(output_g + features)   # [Ng, C]

        # =================== 2) query gauss: Q–G cross-attn ===================
        query_out = None
        if query_features is not None and query_means is not None:
            Nq = query_features.shape[0]
            if Nq > 0:
                # 2.1 query → gauss 的 knn（用 FAISS，简单稳定）
                knn_indices_q, valid_mask_q = self.build_cross_knn_graph(
                    query_means=query_means, key_means=means
                )
                kq = knn_indices_q.shape[1]

                # 2.2 query 的 Q，gauss 的 K,V
                Q_q = self.q_proj(query_features).contiguous().reshape(Nq, self.num_heads, self.head_dims)
                # K_g, V_g 已经在上面算过，可以直接复用

                K_neighbors_q = self.gather_neighbors(K_g, knn_indices_q)  # [Nq, H, kq, D]
                V_neighbors_q = self.gather_neighbors(V_g, knn_indices_q)  # [Nq, H, kq, D]

                Q_expanded_q = Q_q.unsqueeze(2)  # [Nq, H, 1, D]

                attn_logits_q = torch.bmm(
                    Q_expanded_q.reshape(Nq * self.num_heads, 1, self.head_dims),
                    K_neighbors_q.reshape(Nq * self.num_heads, kq, self.head_dims).transpose(1, 2)
                ).reshape(Nq, self.num_heads, 1, kq)

                attn_logits_q = attn_logits_q / (self.head_dims ** 0.5)

                valid_mask_expanded_q = valid_mask_q.unsqueeze(1).unsqueeze(2)  # [Nq,1,1,kq]
                NEG_INF_Q = torch.finfo(attn_logits_q.dtype).min / 2 if attn_logits_q.dtype in [torch.float16, torch.bfloat16] else -1e9
                attn_logits_q = attn_logits_q.masked_fill(~valid_mask_expanded_q, NEG_INF_Q)

                attn_weights_q = F.softmax(attn_logits_q, dim=-1)
                attn_weights_q = self.dropout(attn_weights_q)

                output_q = torch.bmm(
                    attn_weights_q.reshape(Nq * self.num_heads, 1, kq),
                    V_neighbors_q.reshape(Nq * self.num_heads, kq, self.head_dims)
                ).reshape(Nq, self.num_heads, self.head_dims)

                output_q = output_q.contiguous().reshape(Nq, self.embed_dims)
                output_q = self.output_proj(output_q)
                output_q = self.dropout(output_q)
                # 注意：残差是加回 query_features（不是 gauss_out）
                query_out = self.norm(output_q + query_features)   # [Nq, C]

        return gauss_out, query_out
...
class GaussianTPVCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        # Attention weights: per-level computation（固定 anchor 数）
        self.attention_weights = nn.Linear(embed_dims, num_heads * num_anchors * num_points)
        
        # Sampling offsets
        if use_offset:
            self.sampling_offsets = nn.Linear(embed_dims, num_heads * num_anchors * num_points * 2)
    ...
    def init_weights(self):
        ...
        if self.use_offset:
            ...
            for i in range(self.num_points):
                grid_init[:, i, :] *= i + 1
            grid_init = grid_init.repeat(1, self.num_anchors, 1)
    ...
    def _deform_attend_single_level(
        ...
        assert num_anchors_l == self.num_anchors, \
            f"num_anchors_l={num_anchors_l} != self.num_anchors={self.num_anchors}"
        
        attn_weights_logits = self.attention_weights(query).view(
            B, Nq, self.num_heads, self.num_anchors, self.num_points
        )
        
        if self.use_offset:
            sampling_offsets = self.sampling_offsets(query).view(
                B, Nq, self.num_heads, self.num_anchors, self.num_points, 2
            )
        else:
...
class GaussianDecoder(nn.Module):
    ...
    - normal gaussian：解码 Δxyz, Δscale, Δrotation, semantic
    - query gaussian：只解码 Δxyz, Δscale, Δrotation（不改语义）

    默认在原字典上 in-place 更新，节省显存。
    """

    def __init__(
        self,
        embed_dims: int,
        pc_range: Optional[List[float]] = None,   # 如果不用裁剪 mu，可以先留着/或者干脆删掉
        scale_range: List[float] = [0.01, 3.2],
        unit_xyz: List[float] = [4.0, 4.0, 2.0],  # 3D 增量范围
    ):
        super().__init__()
        self.embed_dims = embed_dims
        ...
        # normal gaussian: Δxyz(3) + Δscale(3) + Δquat(4) + semantic(4) = 14
        param_dim = 3 + 3 + 4 + 4
        self.param_decoder = MLP(embed_dims, embed_dims * 4, param_dim)

        # query gaussian: Δxyz(3) + Δscale(3) + Δquat(4) = 10
        param_dim_query = 3 + 3 + 4
        self.param_decoder_query = MLP(embed_dims, embed_dims * 4, param_dim_query)
    ...
    def _update_single_gaussian(
        ...
        """
        更新一个 gaussian 字典（in-place），支持：
        - with_semantic=True：解码 semantic 增量
        - with_semantic=False：不解 semantic
        """
        feats = gaussian["features"]  # [N, C]
        params = decoder(feats)       # [N, param_dim]

        if with_semantic:
            # [Δx,Δy,Δz] [Δsx,Δsy,Δsz] [Δquat(4)] [semantic(4)]
            delta_xyz      = params[..., 0:3]
            scale_params   = params[..., 3:6]
            rotation_raw   = params[..., 6:10]
            semantic_delta = params[..., 10:14]
        else:
            # [Δx,Δy,Δz] [Δsx,Δsy,Δsz] [Δquat(4)]
            delta_xyz    = params[..., 0:3]
            scale_params = params[..., 3:6]
            rotation_raw = params[..., 6:10]
            semantic_delta = None
        ...
        # ---- 1. 位置更新 ----
        delta_xyz = (2 * safe_sigmoid(delta_xyz) - 1.0) * self.unit_xyz[None, :]  # 映射到 [-unit, unit]
        gaussian["mu"] = gaussian["mu"] + delta_xyz

        # 可选：如果你希望 mu 也被限制在 pc_range 内，可以加：
        if self.pc_range is not None:
            xyz_min = self.pc_range[:3][None, :]   # [1, 3]
            xyz_max = self.pc_range[3:][None, :]   # [1, 3]
            gaussian["mu"] = torch.clamp(gaussian["mu"], xyz_min, xyz_max)

        # ---- 2. scale 更新 ----
        delta_scales = torch.tanh(scale_params) * 0.1
        gaussian["scale"] = gaussian["scale"] + delta_scales
        gaussian["scale"] = torch.clamp(
            gaussian["scale"],
            self.scale_range[0].item(),
            self.scale_range[1].item(),
        )

        # ---- 3. rotation 更新（四元数乘法）----
        original_rotation = gaussian["rotation"]                # [N, 4]
        delta_quat = F.normalize(rotation_raw, p=2, dim=-1)     # [N, 4]
        new_quat = self._quat_mul(original_rotation, delta_quat)
        gaussian["rotation"] = F.normalize(new_quat, p=2, dim=-1)

        # ---- 4. semantic 更新（只对 normal）----
        if with_semantic and "semantic" in gaussian:
            gaussian["semantic"] = gaussian["semantic"] + semantic_delta
    ...
    def forward(
        ...
        query_gaussian: Optional[Dict[str, torch.Tensor]] = None,
    ) -> Tuple[Dict[str, torch.Tensor], Optional[Dict[str, torch.Tensor]]]:
        """
        Args:
            original_gaussian:
                'mu','scale','rotation','features','semantic'
            query_gaussian (可选):
                'mu','scale','rotation','features'

        Returns:
            updated_original_gaussian, updated_query_gaussian
        """
        # normal gaussians：更新位置、尺度、旋转、语义
        original_gaussian = self._update_single_gaussian(
            original_gaussian,
            self.param_decoder,
            with_semantic=True,
        )

        # query gaussians：更新位置、尺度、旋转，不管语义
        if query_gaussian is not None:
            query_gaussian = self._update_single_gaussian(
                query_gaussian,
                self.param_decoder_query,
                with_semantic=False,
            )

        return original_gaussian, query_gaussian
