...
class GaussianTPVRefiner(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.tpv_feature_dim = model_cfg.get('TPV_FEATURE_DIM', 128)
        self.embed_dims = model_cfg.get('EMBED_DIMS', 256)
        self.k_neighbors = model_cfg.get('K_NEIGHBORS', 16)
        self.num_heads = model_cfg.get('NUM_HEADS', 8)
        self.num_layers = model_cfg.get('NUM_LAYERS', 1)
        self.num_points = model_cfg.get('NUM_POINTS', 5)
        self.num_learnable_pts = model_cfg.get('NUM_LEARNABLE_PTS', 0)
        self.dropout = model_cfg.get('DROPOUT', 0.1)
        self.max_distance = model_cfg.get('MAX_DISTANCE', 10.0)
        self.scale_range = model_cfg.get('SCALE_RANGE', [0.01, 3.2])
        self.unit_xyz = model_cfg.get('UNIT_XYZ', [4.0, 4.0, 2.0])

        pc_range = model_cfg.get('PC_RANGE', [-54.0, -54.0, -5.0, 54.0, 54.0, 3.0])
        ...
        # 4. 高斯-TPV 交叉注意力模块
        # num_anchors = 5 (基础点) + num_learnable_pts (可学习点)
        num_anchors = 5 + self.num_learnable_pts
        self.cross_attention = GaussianTPVCrossAttention(
            ...
            use_offset=True,
            fuse='concat',  # 或者 'sum'
            num_anchors=num_anchors  # 传递实际的anchor数量
        )
        
        # 5. TPV特征投影（如果维度不匹配）
        if self.tpv_feature_dim != self.embed_dims:
            self.tpv_proj = nn.Linear(self.tpv_feature_dim, self.embed_dims)
        else:
    ...
    def forward(
        ...
        if batch_dict.get('fused_agents_gaussians', None) is not None:
            ...
            tpv_value, spatial_shapes, level_start_index = self.tpv_flattener(
                ...
                tpv_xz=batch_dict['fused_agents_tpv_xz'],
                tpv_yz=batch_dict['fused_agents_tpv_yz']
            ...
            for agent in available_agents:
                tpv_value, spatial_shapes, level_start_index = self.tpv_flattener(
                    tpv_xy=batch_dict[agent]['fused_tpv_xy'],
...
class GaussianTPVCrossAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attention_weights = nn.Linear(embed_dims, num_heads * num_anchors * self.num_points)
        # Sampling offsets
        if use_offset:
            self.sampling_offsets = nn.Linear(embed_dims, num_heads * num_anchors * self.num_points * 2)
    ...
    def init_weights(self):
        ...
        if self.use_offset:
            ...
            # Generate grid pattern: [num_heads, num_points, 2]
            thetas = torch.arange(self.num_heads, dtype=torch.float32) * (2.0 * math.pi / self.num_heads)
            grid_init = torch.stack([thetas.cos(), thetas.sin()], -1)  # [num_heads, 2]
            grid_init = (grid_init / grid_init.abs().max(-1, keepdim=True)[0]).view(
                self.num_heads, 1, 2).repeat(1, self.num_points, 1)  # [num_heads, num_points, 2]
            for i in range(self.num_points):
                grid_init[:, i, :] *= i + 1
            # Expand to include num_anchors: [num_heads, num_anchors, num_points, 2]
            grid_init = grid_init.unsqueeze(1).repeat(1, self.num_anchors, 1, 1)  # [num_heads, num_anchors, num_points, 2]
            self.sampling_offsets.bias.data = grid_init.view(-1)  # [num_heads * num_anchors * num_points * 2]
    ...
    def _deform_attend_single_level(
        ...
        # 检查anchor数量是否匹配（如果初始化时传递了正确的num_anchors，应该匹配）
        if num_anchors_l != self.num_anchors:
            raise ValueError(
                f"Anchor count mismatch: reference_points has {num_anchors_l} anchors, "
                f"but GaussianTPVCrossAttention was initialized with num_anchors={self.num_anchors}. "
                f"Please ensure num_anchors=5+num_learnable_pts when initializing."
            )
