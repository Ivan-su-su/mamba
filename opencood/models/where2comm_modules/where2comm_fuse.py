...
class Communication(nn.Module):
    ...
    def forward(self, batch_confidence_maps, B):
        ...
        communication_rates = []
        for b in range(B):
            ori_communication_maps, _ = (
                batch_confidence_maps[b].sigmoid().max(dim=1, keepdim=True)
            )
            if self.smooth:
            ...
            # Ego
            # print("communication_mask[0] ",communication_mask[0])
            # print("communication_mask shape:", communication_mask.shape)
            communication_mask[0] = (
                1  # batch_size 只能是1，不然这里就会报错，因为batch_confidence_maps拆分后，L2=0
            )
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(cav_num, C, -1).permute(
            2, 0, 1
        )  # (H*W, cav_num, C), perform self attention on each pixel
        x = self.att(x, x, x)
                ...
                if i == 0:
                    if self.fully:
                        ...
                        communication_masks, communication_rates = (
                            self.naive_communication(batch_confidence_maps, B)
                        )
